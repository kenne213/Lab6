...
int main() {
	string backgroundFile = "images1/backgrounds/winter.png";
	string foregroundFile = "images1/characters/yoda.png";

	Texture backgroundTex;
	if (!backgroundTex.loadFromFile(backgroundFile)) {
		cout << "Couldn't Load Image" << endl;
		exit(1);
	}
	Texture foregroundTex;
	if (!foregroundTex.loadFromFile(foregroundFile)) {
		cout << "Couldn't Load Image" << endl;
		exit(1);
	}
	...
	for (int y = 0; y < sz.y; y++) {
		for (int x = 0; x < sz.x; x++) {
			Color imagePixel = foregroundImage.getPixel(x, y);
			 
			if (imagePixel == greenColor) {
